...
namespace GameHub.Controllers
{
    public class ShopController : Controller
    {
        ...
        // add configuration dependency so we can read the Stripe key from appsettings or the Azure app vars section
        ...
        {
            // fetch list of categories to display so customer can pick one
        ...
        public IActionResult AddToCart(int id)
        {
            ...
            if (cartItem == null)
            {
                // create and save a new CartItem to the DB
            ...
            {
                cartItem.Quantity += 1; // add 1 to current quantity
        ...
        private string GetCartIdentifier()
        {
            ...
            // we need to use a session var because HTTP is "stateless" - variables do not persist between http requests
            if (HttpContext.Session.GetString("CartIdentifier") == null)
            {
                // if not, use a GUID to create a unique cart id and store in a new session var
        ...
        public IActionResult Cart()
        {
            ...
            // fetch cartItems for display. Join to parent Product so we can also show Product details
            var cartItems = _context.CartItems.Include(c => c.Product).Where(c => c.CustomerId == cartIdentifier).ToList();

            // count items in cart to display in Navbar badge
            ...
            var cartTotal = (from c in cartItems
        ...
        public IActionResult Category(string Name)
        {
            ...
            // use the Product model to create a list of products in memory for display
            var products = new List<Product>();

            // use a loop to make some products
            for (var i = 1; i < 16; i++)
            {
        ...
        // GET: /Shop/RemoveFromCart/5 => delete this item from the current cart
        ...
        // GET: /Shop/Checkout => show checkout form.  only for authenticated users
        [Authorize]
        ...
        // POST: /Shop/Checkout => save all order & customer data to a session var
        [Authorize]
        [HttpPost]
        public IActionResult Checkout([Bind("FirstName,LastName,Address,City,Province,PostalCode,Phone")] Order order)
        {
            ...
            // .NET sessions only support Int and String so we need to add an extension library
